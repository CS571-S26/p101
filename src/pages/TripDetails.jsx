...
import {
  ...
  UserPlus, Pencil, ChevronLeft, X, Crown, Mail, Users, Compass,
} from 'lucide-react';
      ...
      <div className="tripdetails-page">
        ...
        <div className="td-main">
          ...
          {activeNav === 'budget' && (
            <BudgetTracker
              tripId={
                (tripData.title || tripData.destination || 'default-trip')
                  .toLowerCase()
                  .replace(/[^a-z0-9]+/g, '-')
                  .replace(/(^-|-$)/g, '')
              }
              members={members.map((m, i) => ({ id: String(i + 1), name: m.name, color: m.color }))}
              totalBudget={
                tripData.budget
                  ? (typeof tripData.budget === 'number'
                      ? tripData.budget
                      : parseInt(String(tripData.budget).replace(/[^0-9]/g, ''), 10) || 5000)
                  : 5000
              }
            />
          )}
          {activeNav === 'experiences' && (
            <div className="td-placeholder">
              <Compass size={40} />
              <h3>Experiences</h3>
              <p>Discover and save local experiences for your trip</p>
            </div>
          )}
          {activeNav === 'map' && (
